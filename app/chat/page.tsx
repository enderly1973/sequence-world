...
export default function ChatPage() {
  ...
  const [hierarchyRooms, setHierarchyRooms] = useState<HierarchyChatRoom[]>([]);
  ...
  async function loadPrivateRooms(userId: string) {
    const { data: roomData, error: roomError } = await supabase
      ...
      .or(`master_id.eq.${userId},slave_id.eq.${userId}`);
    ...
    const result: PrivateChatRoom[] = await Promise.all(
      roomData.map(async (room) => {
        const isMaster = room.master_id === userId;
  ...
  async function loadHierarchyRooms(userId: string) {
    const { data: roomData, error: roomError } = await supabase
    ...
    const superiorIds = [
      ...new Set(roomData.map((room) => room.superior_id)),
    ];
    ...
    const result: HierarchyChatRoom[] = await Promise.all(
      roomData.map(async (room) => {
        ...
        return {
          id: room.id,
          superior_id: room.superior_id,
          superior_name:
            nameMap.get(room.superior_id) ?? "未知上級",
