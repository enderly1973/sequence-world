...
type TaskStatus = "pending" | "accepted" | "submitted" | "completed" | "cancelled";
...
type FilterType = "all" | "pending" | "accepted" | "submitted" | "overdue" | "completed" | "cancelled";
...
export default function AdminTasksPage() {
  ...
  async function loadTasks() {
    ...
    try {
      ...
      const { data: profileData, error: profileError } = await supabase
        .from("profiles")
        .select("id, nickname, join_sequence")
        .in("id", ids);
      ...
      setTasks(
        rows.map((task) => ({
          task,
          sender: profileMap.get(task.sender_id) ?? null,
          receiver: profileMap.get(task.receiver_id) ?? null,
        }))
      );
    } catch (error) {
      setErrorMessage(
  ...
  const counts = useMemo(
    () => ({
      all: tasks.length,
      pending: tasks.filter((item) => item.task.status === "pending").length,
      accepted: tasks.filter((item) => item.task.status === "accepted").length,
      submitted: tasks.filter((item) => item.task.status === "submitted").length,
      overdue: tasks.filter((item) => isTaskOverdue(item.task)).length,
      completed: tasks.filter((item) => item.task.status === "completed").length,
      cancelled: tasks.filter((item) => item.task.status === "cancelled").length,
    }),
    [tasks]
  );
  ...
  const filteredTasks = useMemo(() => {
    const sorted = [...tasks].sort((a, b) => {
      const aSubmitted = a.task.status === "submitted";
      const bSubmitted = b.task.status === "submitted";
    ...
    if (filter === "overdue") {
      return sorted.filter((item) => isTaskOverdue(item.task));
    }

    return sorted.filter((item) => item.task.status === filter);
  }, [tasks, filter]);
  ...
  function getStatusLabel(status: TaskStatus) {
    if (status === "pending") return "等待接受";
    if (status === "accepted") return "進行中";
    if (status === "submitted") return "待確認";
    if (status === "completed") return "已完成";
    return "已取消";
  }

  function getStatusClass(status: TaskStatus) {
    if (status === "pending") return "border-amber-900 text-amber-300";
    if (status === "accepted") return "border-blue-900 text-blue-300";
    if (status === "submitted") return "border-violet-900 text-violet-300";
    if (status === "completed") return "border-emerald-900 text-emerald-300";
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-5">
            <p className="text-sm text-neutral-500">全部任務</p>
          ...
          <div className="rounded-2xl border border-violet-900/60 bg-violet-950/20 p-5">
            <p className="text-sm text-violet-400">待確認</p>
          ...
          <div className="rounded-2xl border border-red-900/60 bg-red-950/20 p-5">
            <p className="text-sm text-red-400">已逾期</p>
          ...
          <div className="rounded-2xl border border-emerald-900/60 bg-emerald-950/20 p-5">
            <p className="text-sm text-emerald-400">已完成</p>
        ...
        <section className="mb-6 rounded-2xl border border-neutral-800 bg-neutral-900 p-4">
          <div className="flex flex-wrap gap-2">
            {[
              ["all", `全部 ${counts.all}`],
              ["pending", `等待接受 ${counts.pending}`],
              ["accepted", `進行中 ${counts.accepted}`],
              ["submitted", `待確認 ${counts.submitted}`],
        ...
        <section className="rounded-2xl border border-neutral-800 bg-neutral-900 p-6">
          {filteredTasks.length === 0 ? (
            ...
            <div className="space-y-4">
              {filteredTasks.map(({ task, sender, receiver }) => {
                ...
                return (
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-5">
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-sm text-neutral-500">
                          <span>
                            發送者：
                            {sender
                              ? `${sender.nickname}・${formatSequence(sender.join_sequence)}`
                              : "未知成員"}
                          </span>
                          ...
                          <span>
                            接收者：
                            {receiver
                              ? `${receiver.nickname}・${formatSequence(receiver.join_sequence)}`
                              : "未知成員"}
                          </span>

                          <span>建立：{formatDate(task.created_at)}</span>

                          <span
                            className={
                              overdue ? "font-medium text-red-400" : undefined
                            }
                          >
                      ...
                      >
                        {overdue ? "已逾期" : getStatusLabel(task.status)}
                      </span>
                    </div>
                  </Link>
                );
